...
import datetime
import os
import tools # Подключаем наш файл с математикой

# --- Константы интерфейса ---
WIDTH, HEIGHT = 1200, 800
CANVAS_WIDTH = 950 # Область рисования меньше общей ширины, чтобы осталось место под меню
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
DARK_GRAY = (40, 40, 40)
...
class PaintApp:
    def __init__(self):
        ...
        pygame.display.set_caption("Ultimate Paint Pro 2026")
        
        # Создаем холст (отдельная поверхность, чтобы меню не затиралось при рисовании)
        ...
        self.text_content = ""
        self.text_pos = (0, 0)
        self.color_rects = [] # Список для хранения координат кнопок палитры

    def draw_sidebar(self):
        """Рисует правую панель управления и подсказки"""
        ...
        y_offset += 30
        self.color_rects = [] # Очищаем список перед перерисовкой
        for i, col in enumerate(COLORS):
            rect = pygame.Rect(CANVAS_WIDTH + 20, y_offset, 25, 25)
            # Обводка активного цвета
            if self.color == col and self.tool != "eraser":
                pygame.draw.rect(self.screen, WHITE, rect.inflate(4, 4), 2)
            pygame.draw.rect(self.screen, col, rect)
            self.screen.blit(self.font_main.render(COLOR_NAMES[i], True, LIGHT_GRAY), (CANVAS_WIDTH + 55, y_offset + 3))
            self.color_rects.append((rect, col))
            y_offset += 35

    def save_image(self):
        """Сохраняет холст в папку assets рядом с файлом программы"""
        ...
        save_path = os.path.join(assets_dir, f"art_{timestamp}.png")
    ...
    def draw_shape_to(self, surf, tool, start, end, color, thick):
        """Универсальная функция отрисовки фигур на любой поверхности"""
    ...
    def run(self):
        ...
        while True:
            ...
            self.screen.blit(self.canvas, (0, 0)) # Рисуем наш холст
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    # Ввод текста
                    if self.text_active:
                        if event.key == pygame.K_RETURN: # Зафиксировать текст
            ...
            # Логика непрерывного рисования (Карандаш/Ластик)
            ...
            # Предпросмотр фигуры во время перетаскивания (на экране, не на холсте)
